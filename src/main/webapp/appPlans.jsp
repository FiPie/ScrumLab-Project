    ...
    <div class="row dashboard-nowrap">
        ...
        <div class="m-4 p-3 width-medium">
            <div class="dashboard-content border-dashed p-3 m-4 view-height">
                ...
                <sql:query dataSource="${con}" sql="SELECT * FROM plan WHERE admin_id= ${sessionScope.id}"
                           var="result"/>
                ...
                <div class="schedules-content">
                    <table class="table border-bottom">
                        ...
                        <c:forEach var="rows" items="${result.rows}">
                            ...
                            <tr class="d-flex">
                                <td class="col-1">${rows.id}</td>
                                <td class="col-2">${rows.name}</td>
                                <td class="col-7">${rows.description}</td>
                                <td class="col-2 d-flex align-items-center justify-content-center flex-wrap">

                                    <form action="/app/plan/delete" method="post">
                                        <button type="submit" id="planIdtoDelete"
                                                name="planIdtoDelete" value="${rows.id}"
                                                class="btn btn-danger rounded-0 text-light m-1">Usuń
                                        </button>
                                    </form>


                                    <form action="/app/plan/details" method="get">
                                        <button type="submit" id="planIdDetails"
                                                name="planIdDetails" value="${rows.id}"
                                                class="btn btn-info rounded-0 text-light m-1">Szczegóły
                                        </button>
                                    </form>


                                    <form action="/app/plan/edit" method="get">
                                        <button type="submit" id="planId" name="planId"
